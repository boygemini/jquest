...
export const disableButtons = (t) => {
	buttons.forEach((e) => {
		t && (e.disabled = !0), t || (e.disabled = !1);
	});
};
changeEmailBox.addEventListener("click", (t) => {
...
export function setStage(t) {
	sessionStorage.setItem("form-stage", t.toString());
}
setStage(0);
...
function reset() {
	Object.entries(USERS_FILE)
		.map((t) => t[1])
		.filter((t) => Object.keys(t).length > 0).length > 1 &&
		(animateErrorMessage(
			3e3,
			600,
			20,
...
[textFieldLabels, textInputFields].forEach((t) => {
	t.forEach((t) => {
		["click", "focus"].forEach((e) => {
			t.addEventListener(e, (t) => {
				"LABEL" === t.target.nodeName &&
					(t.target.nextElementSibling.focus(),
					t.target.nextElementSibling.click()),
					"INPUT" === t.target.nodeName &&
						((t.target.offsetParent.children[1].style.borderBottom =
							"1px solid #2260ff"),
						(t.target.offsetParent.firstElementChild.classList += " move-up"));
			});
		});
	}),
		t.forEach((t) => {
			t.addEventListener("blur", (t) => {
				if (t.target.classList.value.includes("ads"))
					try {
						0 === t.target.value.trim().length &&
							(t.target.offsetParent.firstElementChild.classList.remove(
								"move-up"
							),
							(t.target.offsetParent.children[1].style.borderBottom = ""));
					} catch (t) {}
	...
	textInputFields[0].value.length > 0 &&
		((textInputFields[0].offsetParent.firstElementChild.classList +=
			" move-up"),
		(textInputFields[0].offsetParent.children[1].style.borderBottom =
			"1px solid #2260ff"));
export function animateErrorMessage(t, e, n, s, a, o) {
	errorText.innerText = s;
	const i = errorText.scrollWidth + 30;
	let r = ["remove-error-message", "remove-reset-message"];
	for (let t of r) errorMessage.classList.remove(t);
	errorMessage.classList.add(a),
		window.screen.availWidth <= 1024 &&
			gsap.to(".circle-progress", { opacity: 0, duration: 0.3 }),
		window.screen.availWidth >= 1024 && (errorMessage.style.width = `${n}px`),
	...
	debounce(() => {
		...
		debounce(() => {
			for (let e of t) errorMessage.classList.remove(e);
	...
	debounce(() => {
		window.screen.availWidth >= 1024 && (errorMessage.style.width = "auto"),
			window.screen.availWidth < 1024 && (errorMessage.style.width = "100%"),
			userEmail.classList.remove("empty-email-field");
	}, t + e)();
}
function showSending(t) {
	if (
...
function animateProgress(t) {
	let e = Object.entries(USERS_FILE)
			.map((t) => t[1])
			.filter((t) => Object.keys(t).length > 0),
		n = Object.keys(USERS_FILE).length,
		s = Math.floor((e.length / n) * 100),
		a = Math.floor((s / 100) * 502);
	(progressSVG.style.transition = ".5s"),
		(progressSVG.style.strokeDashoffset = 502 - a),
		(progress.innerHTML = `${s}%`),
		t >= Questions.length
			? (currentStage.innerHTML = `${t - 1} / ${n}`)
			: (currentStage.innerHTML = `${t} / ${n}`);
}
function sendEmail(t) {
	if (
		...
		Object.entries(USERS_FILE)
			.map((t) => t[1])
			.filter((t) => 0 === Object.keys(t).length).length
	) {
		let e = Object.values(USERS_FILE),
			n = [];
		for (let t in e)
			n =
				"2" === t || "8" === t || "9" === t
					...
					: [...n, Object.values(e[t])[0].answerText];
		let s = "";
		const a = (t) =>
			2 === t || 3 === t ? Questions[t].question + n[0] : Questions[t].question;
		...
		let i = { my_html: o, user: t, reply_to: "gbadebojubril@gmail.com" };
		debounce(() => {
			accountRotator(i);
		}, 2e3)();
	}
...
function gotoNextStep(t, e) {
	const n = document.querySelector(".answer-field"),
		s = document.querySelector(".Question");
	if (0 === t) {
		let e = 0 === userEmail.value.trim().length;
		if (e)
			return (
		...
		if (!e) {
			if (!validateEmail(userEmail.value.trim(), t))
	...
	if (
		...
		try {
			if (
				Object.keys(USERS_FILE[t - 1]) &&
				0 === Object.keys(USERS_FILE[t - 1]).length
			)
		...
		t <= e.length - 1 && t > 1 && continueDuringSurvey(s, n);
		debounce(() => {
			e[t] &&
				((s.innerHTML =
					`<h1 class="qnumbering">Question ${t} of ${e.length - 1}</h1>` +
					askQuestionsInteractively(t, e)),
				displayAnswersInteractively(n, t, e));
		}, 500)();
	}
	setStage(t), animateProgress(currentFormStage());
}
function gotoPreviousStep(t, e) {
	const n = document.querySelector(".answer-field"),
		s = document.querySelector(".Question");
	if (
		(--t <= 0 && (t = 0),
		...
		t === parseInt(Object.keys(USERS_FILE).length) &&
			(backToForm(startQuestion, s, n), body.classList.remove("blurbody")),
		t <= 0)
	) {
		thankYou.style.display = "none";
		return (
	...
	t > 0 &&
		(t < parseInt(Object.keys(USERS_FILE).length) && continueDuringSurvey(s, n),
		setTimeout(() => {
			(ContinueButton.style.display = "flex"),
				(BackButton.style.display = "flex"),
				BackButton.classList.remove("widen"),
...
function markAlreadyChosenSelections(t) {
	const e = document.querySelectorAll(".ans-box");
	let n = Object.entries(USERS_FILE[t]).map((t) => t[0]);
	e.forEach((t) => {
		n.filter((e) => {
			t.dataset.id === e &&
...
function askQuestionsInteractively(t, e) {
	switch (t) {
		case 2:
		case 3:
			let n = [];
			for (let t in USERS_FILE[1]) n.push(USERS_FILE[1][t].answerText);
			return `${e[t].question} ${n[0]} ?`;
		default:
			return e[t].question;
	}
}
function displayAnswersInteractively(t, e, n) {
	function s(n, s) {
		(t.innerHTML = ""),
			setTimeout(() => {
				switch (e) {
				...
				switch (e) {
					...
					case 9:
					case 10:
						t.classList.add("eight-ans-grid");
						break;
					case 3:
						t.classList.add("icon-text-ans");
						break;
					case 13:
						t.classList.add("continous-two");
						break;
					case 4:
					case 5:
					case 6:
					...
					case 14:
					case 15:
						t.classList.add("ans-with-4ans-textonly");
						break;
					case 16:
					case 17:
					case 18:
					case 12:
						document
							...
							.forEach((t) => t.classList.add("five-texts-ans")),
							t.classList.add("ans-with-5option-textonly");
						break;
					case 19:
					case 20:
					case 2:
						t.classList.add("straight-ans");
				}
				handleAnswers(e);
			}, 0);
	}
	if (
		([
			...
			t.classList.value.includes(e) && t.classList.remove(e);
		}),
		n[e].answers[0].text &&
			n[e].answers.forEach((t, e) => {
				s(e, t);
			}),
		!n[e].answers[0].text)
	) {
		function a(t) {
			n[e].answers[t].forEach((t, e) => {
				s(e, t);
			});
		}
...
const addCheckAnimationForImgTextBox = (t) => {
		t.classList.add("checked2");
	},
	removeCheckAnimationForImgTextBox = (t) => {
		t.classList.remove("checked2");
	},
	addCheckAnimationForTextBoxOnly = (t, e) => {
		let n = e;
		t.classList.add("text-checked"), n.classList.add("textcontainer-checked");
	},
	removeCheckAnimationForTextBoxOnly = (t, e) => {
		let n = e;
		t.classList.remove("text-checked"),
			n.classList.remove("textcontainer-checked");
	},
	addCheckAnimationForIconAndText = (t, e) => {
		let n = e;
		t.classList.add("opacity"), n.classList.add("tools-ans-checked");
	},
	removeCheckAnimationForIconAndText = (t, e) => {
		let n = e;
		t.classList.remove("opacity"), n.classList.remove("tools-ans-checked");
	},
	addCheckAnimationForTwoAns = (t, e) => {
		let n = e;
		t.classList.add("opacity"), n.classList.add("two-ans-container-checked");
	},
	removeCheckAnimationForTwoAns = (t, e) => {
		let n = e;
		t.classList.remove("opacity"),
			n.classList.remove("two-ans-container-checked");
	};
function addClass(t) {
	let e = [
		t.target,
		...
		t.target.parentElement.parentElement.parentElement,
	];
	for (let t in e) {
		if (e[t].classList.value.includes("text-imgbox")) {
			addCheckAnimationForImgTextBox(e[t].children[0], e[t]);
			break;
		}
		if (e[t].classList.value.includes("textonlybox")) {
			addCheckAnimationForTextBoxOnly(e[t].children[0], e[t]);
			break;
		}
		if (e[t].classList.value.includes("bare-ans")) {
			addCheckAnimationForIconAndText(e[t].children[0], e[t]);
			break;
		}
		if (e[t].classList.value.includes("two-ans")) {
			addCheckAnimationForTwoAns(e[t].children[0], e[t]);
			break;
		}
	}
}
function removeClass(t) {
	...
	for (let t in e) {
		if (e[t].classList.value.includes("text-imgbox")) {
			removeCheckAnimationForImgTextBox(e[t].children[0], e[t]);
			break;
		}
		if (e[t].classList.value.includes("textonlybox")) {
			removeCheckAnimationForTextBoxOnly(e[t].children[0], e[t]);
			break;
		}
		if (e[t].classList.value.includes("bare-ans")) {
			removeCheckAnimationForIconAndText(e[t].children[0], e[t]);
			break;
		}
		if (e[t].classList.value.includes("two-ans")) {
			removeCheckAnimationForTwoAns(e[t].children[0], e[t]);
			break;
		}
	}
}
function markAndSaveSelections(t, e, n) {
	t ||
		...
		e.forEach((s) => {
			s.addEventListener("click", (s) => {
				...
				e.forEach((t) => {
					t.classList.value.includes("text-imgbox")
						? removeCheckAnimationForImgTextBox(t.children[0], t)
						: t.classList.value.includes("textonlybox")
						? removeCheckAnimationForTextBoxOnly(t.children[0], t)
						: (t.classList.value.includes("bare-ans") &&
								removeCheckAnimationForIconAndText(t.children[0], t),
						  t.classList.value.includes("two-ans") &&
								removeCheckAnimationForTwoAns(t.children[0], t));
				}),
					addClass(s),
					1 === n && reset(),
					addAnswerToPersonObject(t, a.dataset.id, a.dataset.answer, n);
			});
		})),
		t &&
			...
			e.forEach((e) => {
				e.addEventListener("click", (e) => {
					...
					for (let a in s) {
						if (
							(s[a].classList.value.includes("text-imgbox") &&
								s[a].children[0].classList.value.includes("checked2")) ||
							(s[a].classList.value.includes("textonlybox") &&
								s[a].children[0].classList.value.includes("text-checked")) ||
							(s[a].classList.value.includes("bare-ans") &&
								s[a].children[0].classList.value.includes("opacity")) ||
							(s[a].classList.value.includes("two-ans") &&
								s[a].children[0].classList.value.includes("opacity"))
						) {
							removeClass(e), removeAnswerToPersonObject(s[a].dataset.id, n);
							break;
						}
						((s[a].classList.value.includes("bare-ans") &&
							!s[a].children[0].classList.value.includes("opacity")) ||
...
function moreThanOneSelectionIsPermitted(t) {
	switch (t) {
		case 3:
		case 9:
		case 10:
...
function handleAnswers(t) {
	const e = document.querySelectorAll(".ans-box"),
		n = moreThanOneSelectionIsPermitted(t);
	document.querySelectorAll(".end").forEach((t) => {
		t.disabled = !0;
	}),
		t <= Questions.length - 1 &&
			...
			(thankYou.style.display = "none"),
			markAlreadyChosenSelections(t),
			markAndSaveSelections(n, e, t));
}
function removeAnswerToPersonObject(t, e) {
	delete USERS_FILE[e][t];
}
function addAnswerToPersonObject(t, e, n, s) {
	t && (USERS_FILE[s][e] = { ...USERS_FILE[s][e], answerText: n }),
		t || (USERS_FILE[s] = { [e]: { answerText: n } });
}
	...
	!0 !== otfp &&
		(currentFormStage() !== Object.values(USERS_FILE).length + 1
			? currentFormStage() <= Object.values(USERS_FILE).length &&
			  currentFormStage() > 0 &&
			  ("ArrowRight" === t.key && gotoNextStep(currentFormStage(), Questions),
			  "ArrowLeft" === t.key &&
					gotoPreviousStep(currentFormStage(), Questions))
			: "ArrowLeft" === t.key &&
			  gotoPreviousStep(currentFormStage(), Questions));
}
function enterKeyPressHandler(t) {
	currentFormStage() < Object.values(USERS_FILE).length + 1 &&
		"Enter" === t.key &&
	...
	cancel.forEach((t) => {
		t.onclick = (t) => {
			...
			debounce(() => {
				...
				t.target.classList.value.includes("c2") &&
					setStage(currentFormStage() - 1);
		};
	}),
	ContinueButton.addEventListener("click", (t) => {
		debounce(() => gotoNextStep(currentFormStage(), Questions), 100)();
	}),
	BackButton.addEventListener("click", (t) => {
		debounce(() => gotoPreviousStep(currentFormStage(), Questions), 200)();
	}),
